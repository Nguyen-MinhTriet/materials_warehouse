...
@section('content')
    <form action="{{ route('batches.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="form-group">
            ...
            <select name="material_id" class="form-control">
                @foreach ($materials as $vt)
                    <option value="{{ $vt->id }}" {{ old('material_id') == $vt->id ? 'selected' : '' }}>
                        {{ $vt->name }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="form-group">
            ...
            <select name="import_receipt_id" class="form-control">
                @foreach ($import_receipts as $px)
                    <option value="{{ $px->id }}" {{ old('import_receipt_id') == $px->id ? 'selected' : '' }}>
                        {{ $px->issued_date }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="mb-3">
            ...
            <input class="form-control" id="example-number" type="number" name="stock_quantity">
