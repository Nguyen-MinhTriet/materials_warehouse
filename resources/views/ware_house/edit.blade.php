...
@section('content')
    <form action="{{ route('warehouses.update', $each) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-3">
            ...
            <input type="text" id="example-helping" name="longitude" class="form-control" placeholder="Helping text" value="{{ $each->longitude }}">
        </div>
        <div class="mb-3">
            <label for="example-helping" class="form-label">Vĩ độ</label>
            <input type="text" id="example-helping" name="latitude" class="form-control" placeholder="Helping text" value="{{ $each->latitude }}">
        </div>
