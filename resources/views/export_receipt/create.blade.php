...
@section('content')
    <form action="{{ route('export_receipts.store') }}" method="POST">
        @csrf
                    ...
                    <select name="payment_method_id" class="form-control">
                        @foreach ($payment_methods as $pttt)
                            <option value="{{ $pttt->id }}"
                                {{ old('payment_method_id') == $pttt->id ? 'selected' : '' }}>
                                {{ $pttt->name }}
                            </option>
                        @endforeach
                    </select>
                    ...
                    <select name="warehouse_id" class="form-control">
                        @foreach ($warehouses as $kholua)
                            <option value="{{ $kholua->id }}"
                                {{ old('warehouse_id') == $kholua->id ? 'selected' : '' }}>
                                {{ $kholua->name }}
                            </option>
                        @endforeach
                    </select>
        ...
        <table class="table table-bordered" id="invoice-details">
            ...
            <tbody>
                <tr class="detail-row">
                    ...
                    <td>
                        <select name="details[0][batch_id]" class="form-control batch-id">
                            <option value="">Chọn lô</option>
                            @foreach ($batches as $lh)
                                <option value="{{ $lh->id }}" {{ old('batch_id') == $lh->id ? 'selected' : '' }}>
                                    {{ $lh->name }}
                                </option>
                            @endforeach
                        </select>
                        @error('details.0.batch_id')
                            <div class="alert alert-danger mt-2">{{ $message }}</div>
                        @enderror
                    </td>
                    <td>
                    ...
                    <td>
                        <input type="number" name="details[0][total_price]" class="form-control total-price" readonly>
