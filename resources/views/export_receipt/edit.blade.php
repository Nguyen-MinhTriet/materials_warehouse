    ...
    <form action="{{ route('export_receipts.update', $each) }}" method="POST">
            ...
            <div class="col-md-6">
                <div class="form-group">
                    ...
                    <select name="employee_id" class="form-control">
                        @foreach ($employees as $nv)
                            <option value="{{ $nv->id }}"
                                {{ old('employee_id', $each->employee_id) == $nv->id ? 'selected' : '' }}>
                                {{ $nv->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select name="payment_method_id" class="form-control">
                        @foreach ($payment_methods as $pttt)
                            <option value="{{ $pttt->id }}"
                                {{ old('payment_method_id', $each->payment_method_id) == $pttt->id ? 'selected' : '' }}>
                                {{ $pttt->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select name="warehouse_id" class="form-control">
                        @foreach ($warehouses as $kholua)
                            <option value="{{ $kholua->id }}"
                                {{ old('warehouse_id', $each->warehouse_id) == $kholua->id ? 'selected' : '' }}>
                                {{ $kholua->name }}
                            </option>
                        @endforeach
                    </select>
                    ...
                    <select name="customer_id" class="form-control">
                        @foreach ($customers as $customer)
                            <option value="{{ $customer->id }}"
                                {{ old('customer_id', $each->customer_id) == $customer->id ? 'selected' : '' }}>
                                {{ $customer->name }}
                            </option>
                        @endforeach
                    </select>
        ...
        <table class="table table-bordered" id="invoice-details">
            ...
            @foreach ($each->details as $index => $detail)
                <tr class="detail-row">
                    ...
                    <td>
                        <select name="details[{{ $index }}][batch_id]" class="form-control batch-id">
                            <option value="">Chọn lô</option>
                            @foreach ($batches as $lh)
                                <option value="{{ $lh->id }}"
                                    {{ old('details.' . $index . '.batch_id', $detail->batch_id) == $lh->id ? 'selected' : '' }}>
                                    {{ $lh->name }}
                                </option>
                            @endforeach
                        </select>
                        @error('details.' . $index . '.batch_id')
                            <div class="alert alert-danger mt-2">{{ $message }}</div>
                        @enderror
                    </td>
                    <td>
                    ...
                    <td>
                        <input type="number" name="details[{{ $index }}][total_price]"
                            class="form-control total-price" readonly
                            value="{{ old('details.' . $index . '.total_price', $detail->total_price) }}">
        ...
        <div class="form-group mt-3">
            ...
            <input type="number" name="total_amount" class="form-control" readonly
                value="{{ old('total_amount', $each->total_amount) }}">
            @error('total_amount')
                <div class="alert alert-danger mt-2">{{ $message }}</div>
            @enderror
        </div>
            ...
            $(document).on('click', '#add-row', function() {
                ...
                newRow.find('select').each(function() {
                    ...
                    if ($(this).hasClass('material-id')) {
                        ...
                        @endforeach
                    } else if ($(this).hasClass('batch-id')) {
                        @foreach ($batches as $lh)
                            $(this).append(
